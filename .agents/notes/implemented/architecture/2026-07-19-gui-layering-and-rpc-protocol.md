...
- `apps/` holds the externally exported applications, assembled from Client / Host mixtures.
    ...
    - `apps/desktop` supervises `dsh web` and loads its loopback origin; the [desktop decision](../feature/2026-08-14-electron-desktop-shell.md) keeps the reserved IPC carrier as a later transport option.
...
| Carrier layer | `dsh-host-webserver` | Web HTTP and upgrade: static serving + `/api/*`→handler forwarding + WebSocket upgrade route + close semantics; plugin bundle endpoint + `__DSH_BOOT__` manifest injection (fed by the web plugin registry) | Browser delivery over HTTP; zero workspace dependencies (the registry arrives by structural injection); the desktop shell reaches it through its supervised Web Host |
...
1. **Pick a fetch impersonation**: browser same-origin HTTP / in-process `host.handler.fetch` injection / your own transport-aspect subclass (the reserved Electron IPC option is in the "Subclass table" below).
...
| IPC bridge subclass (reserved; the shipped desktop shell uses the Web carrier) | a future in-process Electron composition | IPC serialization round trip | would swap only doFetch; contract and base class unchanged |
