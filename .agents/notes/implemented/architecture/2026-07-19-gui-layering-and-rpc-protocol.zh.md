...
- `apps/` 作为对外导出的应用入口，可以由 Client / Host 混合组装。
    ...
    - `apps/desktop` 监督 `dsh web` 并加载其 loopback 源；[桌面端决定](../feature/2026-08-14-electron-desktop-shell.zh.md)将预留的 IPC 载体保留为后续传输选项。
...
| 承载层 | `dsh-host-webserver` | Web HTTP 与 upgrade：静态服务 + `/api/*`→handler 转发 + WebSocket upgrade route + close 语义；插件 bundle 端点 + `__DSH_BOOT__` manifest（元数据清单）注入（由 web 插件注册表供给） | 经 HTTP 交付浏览器；零 workspace 依赖（注册表经结构注入到达）；桌面壳通过受监督 Web Host 使用它 |
...
1. **选 fetch 伪造方式**：浏览器同源 HTTP / 进程内 `host.handler.fetch` 注入 / 自写传输切面子类（预留的 Electron IPC 选项见下文「子类表」）。
...
| IPC 桥子类（预留；已交付桌面壳使用 Web 载体） | 将来的进程内 Electron 组合 | IPC 序列化往返 | 只需换 doFetch，约定/基类零改 |
