...
|---|---|---|---|---|
| dsh | 发布集：非 experimental 的 `packages/*/*` 加发布到 npm 的 `apps/*`；私有实验性包与 `@deepseek-ai/dsh-desktop` 仅加入共享版本 bump | 发布集、私有 dsh 包与 workspace 根共用一个 `0.0.x` | `dsh-v<版本>` | `release.yml` |
...
三条序列的 npm 成员都发布到 npmjs.com 的 `@deepseek-ai` scope，且 access 按序列而非按 scope 区分：vendored 框架与 native 包是 `public`，dsh 的 npm 集是 `restricted`（[理由](2026-08-13-public-vendor-and-native-sequences.md)）。没有任何发布路径传 `--access`——一个选项无法服务级别互不相同的序列，且会覆盖真正拥有该级别的 manifest。
...
`release:dsh` 接受 `major`、`minor`、`patch` 或显式版本号，把同一个版本写进可发布族、`packages/*/*` 下的每个私有包、私有桌面成员**以及 workspace 根**。私有包仍位于 pack 与 publish 之外；它们跟随版本是因为 workspace 约束要求每个 dsh 包的版本等于根版本。根的检查接受预发布段。像 `0.0.1-rc.1` 这样的预发布号先把 pack、已安装产物探针和一次真实私有发布跑通，数字版本随后。dist-tag 沿用 `landlock-run-release.yml` 已有的判定：版本带预发布段就 `--tag next`，否则进 `latest`。
...
| `ReleaseMember` | 一个参与版本联动的家族成员：目录、包名、版本、manifest；`publishableMembers` 会在 npm 操作前排除私有成员 |
| `publishOrder` | 按 npm 会安装的依赖段加 peer 声明做拓扑序，同层按包名排；安装依赖成环是报错而不是随意定序，任何排不进去的 peer 边被丢弃并点名 |
| `pack` | 校验整个家族的版本基线，把 npm 成员打进一个目录，并记录上传顺序 |
| `verify` | 校验家族版本基线并完整打印 npm 发布顺序；发布时还要求本次运行来自该族的 tag，且 npm 成员可发布 |
...
| 项 | 内容 |
|---|---|
| npm 发布集 manifest | 去掉 `private: true`；按序列补 `publishConfig.access` 与带各自 `directory` 的 `repository` |
| npm 发布集边界 | `packages/*/*` 和 `vendor/*` 的全部成员，加上发布到 npm 的 `apps/*`；私有 `apps/desktop` 共享 dsh 版本与 tag，但以安装包分发 |
