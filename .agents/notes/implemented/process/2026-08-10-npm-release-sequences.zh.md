...
|---|---|---|---|---|
| dsh | 发布集：非 experimental 的 `packages/*/*` 加发布到 npm 的 `apps/*`；私有实验性包与 `@deepseek-ai/dsh-desktop` 仅加入共享版本 bump | 发布集、私有 dsh 包与 workspace 根共用一个 `0.0.x` | `dsh-v<版本>` | `release.yml`（pack）/ `release-publish.yml`（发布） |
