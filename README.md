...
This repository maintains a personal Desktop distribution that follows the official [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) source and preserves its upstream history together with local Desktop customizations. It is not an official DeepSeek distribution. See the [desktop application documentation](apps/desktop/README.md) for installation ownership, packaging, updates, security, and limitations.
...
- Integrated the official `dsh-v0.1.5-rc.2` baseline while retaining the Desktop profile, bundled runtime, plugin manager, update flow, and compact sidebar release badge.
- Refined message feedback actions and dialogs, including matching positive and negative feedback behavior.
- Refined delivered-file and produced-file cards, file-type icon assets, layout spacing, and release-level browser coverage.
- Restored the repository-owned DeepSeek application icon after the upstream integration replaced it with Electron's default icon.

#### 0.1.5-rc.1 — 2026-09-10

- Replaced the loopback Web Host with the official port-free `dsh-app://` Desktop transport, a version-bound dsh runtime, and an isolated Desktop-owned plugin profile.
- Added the packaged plugin manager, application update flow, startup recovery, signed runtime inventory, and platform-specific release pipelines.
- Added document and image previews, global sidebar panels and tabs, delivered-file cards and native file actions, and message feedback.
- Adopted DeepSeek V4.1 Flash as the default Chat Completions model while retaining the V4 and V4 Flash Vision Exp catalog entries.
