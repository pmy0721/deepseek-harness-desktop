...
桌面壳启动 `dsh web --no-open --port 0`，最多等待 90 秒获取标准就绪行，再加载报告的 `127.0.0.1` 源。`--no-open` 会关闭 CLI 的本机浏览器交接，因为产品窗口由 Electron 管理。超时或就绪行格式错误时会终止该 Host，记录诊断及最近 32 KiB 启动输出，并保持原生窗口开启，由有限指数退避启动替代进程。明确退出应用时先发送 `SIGTERM`，等待五秒，必要时升级为 `SIGKILL`。关闭窗口只会隐藏窗口，托盘继续维持 Host 运行。
