...
function linkDirectory(source: string, destination: string, skipDangling = false): void {
  let resolvedSource: string
  try {
    resolvedSource = realpathSync(source)
  } catch (error) {
    if (skipDangling && (error as NodeJS.ErrnoException).code === 'ENOENT') return
    throw error
  }
  mkdirSync(dirname(destination), { recursive: true })
  symlinkSync(resolvedSource, destination, process.platform === 'win32' ? 'junction' : 'dir')
}
...
function mirrorDependencyLinks(sourceRoot: string, destinationRoot: string): void {
  for (const entry of readdirSync(sourceRoot, { withFileTypes: true })) {
    ...
    if (entry.isDirectory() || entry.isSymbolicLink()) {
      linkDirectory(source, join(destinationRoot, entry.name), entry.isSymbolicLink())
    }
  }
}
