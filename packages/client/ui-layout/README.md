...
When the desktop shell supplies `data-dsh-desktop-platform`, AppFrame gives macOS and Windows a translucent sidebar surface while the conversation and details columns remain opaque. macOS uses a 90px collapsed rail to clear its traffic lights; Windows keeps the ordinary 56px rail. Linux receives only the native-titlebar top inset and otherwise retains the browser surfaces.
