...
桌面壳提供 `data-dsh-desktop-platform` 时，AppFrame 会让 macOS 和 Windows 使用半透明侧边栏表面，而会话栏与详情栏保持不透明。macOS 使用 90px 折叠轨道避让交通灯，Windows 仍使用普通的 56px 轨道。Linux 只接收原生标题栏顶部 inset，其余表面保持浏览器布局。
