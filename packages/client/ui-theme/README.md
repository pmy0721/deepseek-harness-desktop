...
The base stylesheet owns the desktop-window surface variables as well as ordinary browser defaults. A desktop platform marker makes macOS and Windows document roots transparent so Electron can expose native vibrancy or acrylic beneath the sidebar, while AppFrame restores opaque conversation and details surfaces. Every declared title-bar drag region is disabled while a modal is open, preventing window movement from competing with modal interaction.
