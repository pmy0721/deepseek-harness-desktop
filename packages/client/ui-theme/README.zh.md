...
基础样式表同时持有桌面窗口表面变量与普通浏览器默认值。桌面平台标记会让 macOS 与 Windows 的 document root 透明，使 Electron 能在侧边栏下方呈现原生 vibrancy 或 acrylic；AppFrame 随后恢复不透明的会话栏与详情栏。模态框打开时，所有已声明的标题栏拖拽区都会被禁用，避免窗口移动与模态交互发生竞争。
