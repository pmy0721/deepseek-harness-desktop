...
async function collectShell(
  ...
  for (const path of globSync('apps/*/vite.config.ts', { cwd: root }).sort()) {
    ...
    if (firstPage === undefined) throw new Error(`browser notices: ${manifest.name} has no HTML build entry`)
    const htmlInput: string | string[] = pages.length === 1 ? firstPage : pages
    await vite.build({
      root: dir,
      logLevel: 'error',
      plugins: [
        {
          ...
          enforce: 'post',
          configResolved(resolved: ShellConfig) {
            resolved.build.rollupOptions ??= {}
            resolved.build.rollupOptions.input = htmlInput
          },
        },
        recorder(seen, workspaceNames, true),
      ],
